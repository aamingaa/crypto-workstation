...
class GPAnalyzer:
    ...
    def initialize_his_data(self):
        ...
        if not self.data_initialized:
            self.X_all, self.X_train, self.y_train, self.ret_train, self.X_test, self.y_test, self.ret_test, self.feature_names,self.open_train,self.open_test,self.close_train,self.close_test, self.z_index ,self.ohlc= dataload.data_prepare(
                ...
                self.start_date_test, self.end_date_test, rolling_w=self.rolling_window, data_dir=self.data_dir, read_frequency=self.read_frequency, timeframe=self.timeframe)
    ...
    def read_and_cal_metrics(self):
        ...
        z = pd.read_csv('/Users/aming/project/python/gp_crypto_next/gp_models/BTCUSDT_30min_1_2025-01_2025-04_2025-04_2025-05.csv')
