...
def MA_Strategy(data_price, window_short=5, window_median=10, window_long=20, loss_ratio=0.20):
    ...
    for i in range( max(1,window_long),data_price.shape[0]): 
        ...
        if (data_price['position'][i-1] == 0) and (data_price['sma'][i - 1] < data_price['lma'][i - 1]) and (
            ...
            data_price['flag'][i+1] = 1 # 记录做多还是做空，这里1是做多
            data_price['position'][i+1] = 1 # 仓位记录为1，表示有1手仓位，注意这里发生买入的时候，下一个timestamp的return才计入损益表
            ...
            data_price['flag'][i+1] = 1 # 记录做多还是做空，这里1是做多
            data_price['position'][i+1] = 1 # 仓位记录为1，表示有1手仓位
            date_in = data_price.index[i] # 记录买入的时间 年-月-日
            price_in = data_price['close'][i] # 记录买入的价格，这里是以收盘价买入
            entry_index = i
        ...
        # 情形二：当前持仓且下跌超过止损率，则平仓止损
            ...
            data_price['flag'][i+1] = -1 # 记录做多还是平仓，这里-1是做多
            data_price['position'][i+1] = 0 # 仓位记录为0，表示没有仓位了
        ...
        # 情形三：当前持仓且短均线下穿长均线(死叉)，则卖出股票
            ...
            data_price['flag'][i+1] = -1 # 记录做多还是做空，这里-1是做多
            data_price['position'][i+1] = 0 # 仓位记录为0，表示没有仓位了
            date_out = data_price.index[i] # 记录卖出的时间 年-月-日
            price_out = data_price['close'][i] # 记录卖出的价格，这里是以收盘价卖出
        ...
        elif (data_price['position'][i-2] == 1) & (data_price['sma'][i - 2] > data_price['lma'][i - 2]) & (
                data_price['sma'][i-1] < data_price['lma'][i-1]):
            # print(data_price.index[i], '=========死叉卖出')
            data_price['flag'][i+1] = -1 # 记录做多还是做空，这里-1是卖出
            data_price['position'][i+1] = 0 # 仓位记录为0，表示没有仓位了
            date_out = data_price.index[i] # 记录卖出的时间 年-月-日
            price_out = data_price['close'][i] # 记录卖出的价格，这里是以开盘价卖出
            Sell.append([date_out, price_out, '死叉卖出']) # 把卖出记录保存到Sell列表里
...
def show_performance( transactions, strategy):
    ...
    # 按天分组，计算每日收益（当天最后一个净值 / 当天第一个净值 - 1）
    nav_perday = strategy.nav.groupby(pd.Grouper(freq='D')).apply(
        lambda x: x.iloc[-1] / x.iloc[0] - 1 if len(x) > 0 else 0
    )
    benchmark_perday = strategy.benchmark.groupby(pd.Grouper(freq='D')).apply(
        lambda x: x.iloc[-1] / x.iloc[0] - 1 if len(x) > 0 else 0
    )
    # 计算每日超额收益
    excess_ret = nav_perday - benchmark_perday
    
    # 整理每日表现结果
    result_perday = pd.concat([nav_perday, benchmark_perday, excess_ret], axis=1)
    result_perday.columns = ['strategy_ret', 'bench_ret', 'excess_ret']
    result_perday.index.name = 'date'  # 索引名为日期
    print('------------------------------')
    print('每日表现（策略收益/基准收益/超额收益）：')
    print(result_perday)
    print('------------------------------')
